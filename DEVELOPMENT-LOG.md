...
## 36. AI-generated Kids Yoga sessions (self-service, per-trainer) (2026-09-06)

First distinct feature for the Kids Yoga Trainer role (previously identical to a regular trainer). A self-service AI agent generates a full 40-minute, story-driven lesson plan from the user's C-O-S-T-A-R-S prompt, with the six `<INPUT_*>` values chosen from dropdowns rather than typed. Only one Kids Yoga Trainer exists today; capped at 3 generation attempts/day.

**Two-step flow, not one-shot**: `POST /kids-yoga/generate` returns a draft only - nothing is saved. The trainer can generate up to 3 drafts, compare them, then `POST /kids-yoga/freeze` exactly one as the real class for that date. Freezing is what creates the actual `Session` (so it shows up in My Sessions/Dashboard/Super Admin's view like any other session), exports the frozen lesson as a `.txt` file to Google Drive, and saves the `KidsYogaLesson` row. Only one freeze per trainer per date (409 on a second attempt).

- **Schema**: `KidsYogaLesson` (1:1 with `Session`, cascade-deleted with it - `narrative_background`, `primary_theme`, `target_age_range`, `specific_yoga_poses`, `relaxation_setting`, `cultural_context`, the four phase texts, `summary`, `drive_file_id`/`drive_file_link`) and `KidsLessonLog` (one row per *generate* attempt, not freeze - this is what the 3/day cap counts against; carries its own `summary` column so today's earlier drafts feed the same-day anti-repetition check without a second table). Migration `20260906000000_add_kids_yoga_lesson`.
- **Anti-repetition**: every generated draft includes a 1-2 sentence `summary` as a fifth field in the same JSON response (no extra API call). Before generating, `kidsYogaAgent.js`'s `getRecentSummaries` looks up the last 3 *frozen* lessons for this trainer (excluding the date being generated for) plus any of today's own not-yet-frozen drafts, and tells Claude the new story must be meaningfully different from all of them.
- `backend/src/utils/kidsYogaAgent.js` (new, mirrors `aiScheduler.js`'s structure): lazy Anthropic client, `ANTHROPIC_KIDS_LESSON_MODEL` (default `claude-sonnet-5` - real creative long-form writing for children, not the trivial JSON the scheduler produces, so it gets its own stronger model per the established per-feature-model convention), `DAILY_LIMIT = 3` (own constant, independent of the scheduler's cap), `KidsLessonRateLimitError` (own class, mirroring `AiScheduleRateLimitError`).
- **Factored out `backend/src/utils/istDate.js`** (`getIstDayBoundsUtc`, `toIstDateString`) now that two features need the identical IST-day-bounds logic - `aiScheduler.js` updated to import from it instead of its own copy.
- **Google Drive export**: `backend/src/utils/sheets.js` gets a new `uploadKidsYogaLessonFile`, reusing the exact same domain-wide-delegation service-account client already used for sequence spreadsheets (no second Google auth setup) - parented under a new `GOOGLE_KIDS_YOGA_FOLDER_ID` env var. Best-effort: if the Google client isn't configured or the upload fails, it's logged and skipped rather than blocking the freeze - the `Session`/`KidsYogaLesson` still save either way.
- `backend/src/routes/kidsYoga.js` (new, mounted `/api/kids-yoga`): `POST /generate`, `POST /freeze`, `GET /usage` (`requireRole('kids_yoga_trainer')` on all three).
- `backend/src/routes/sessions.js`'s `GET /:id` now includes `kids_yoga_lesson` (1:1, no `serialize` transform needed - passes straight through the existing spread).
- Frontend: `SessionDetail.jsx` renders the four phases + Drive link when `session.kids_yoga_lesson` is present.

Verified locally end-to-end (no live Anthropic calls made yet at this point - that's dev-only, per the "don't spend credits unnecessarily" discipline): migration applied cleanly; `getDailyUsage`/`getRecentSummaries` query shapes run clean against the real dev-schema DB; `/generate` correctly returns 503 when unconfigured and does *not* burn a daily-usage slot on that failure; `/freeze` with a fabricated draft created a real `Session` + `KidsYogaLesson`, uploaded a real `.txt` file to Drive (confirmed via a real `drive_file_id`, then deleted as test cleanup), appeared correctly in `GET /sessions/my` and `GET /sessions/:id`; a second freeze for the same date correctly returned 409. All test data (session, lesson, Drive file, test user) removed afterward. `npm run build` clean on frontend.

### Follow-up after first dev click-through (same day)

Three issues found testing the initial version live:

1. **Kids Yoga classes are actually a recurring Mon-Fri 5 PM slot**, not ad-hoc - the self-service "pick any date/time" model didn't match reality. Rather than building new recurring-schedule code, this reuses the *existing* Weekly Schedule feature (`session-templates`) - Super Admin adds one slot (17:00, weekdays `[1,2,3,4,5]`, topic "Kids Yoga", dedicated to the trainer) exactly like any regular trainer's slot, and the existing rolling-14-day `sessionGenerator.js` auto-creates real, empty `Session` rows for every weekday going forward. The trainer's job becomes filling in that day's content, not creating the session.
2. **The "Generate" action was hard to find** - it lived on a separate "New Session" nav tab, which was also the 7th item competing for space in a bottom nav built for 5-6. Removed that page/route/nav entry entirely; extracted its form+draft UI into a reusable `frontend/src/components/KidsLessonGenerator.jsx` and mounted it inline on `SessionDetail.jsx` - opening any Kids Yoga session that has no lesson yet now shows the generator right there. `MySessions.jsx` also gets a "Needs Lesson" badge on such sessions so they're spottable from the list itself.
3. **Generated lessons were too long/verbose** for a teacher to actually use mid-class. `kidsYogaAgent.js`'s prompt now explicitly asks for tight, scannable cue lines instead of flowing paragraphs, with target word counts per phase (Opening/Closing ~80-120 words, Warmups ~150-200, Narrative Sequence ~300-400); `max_tokens` lowered from 4000 to 2200 to reinforce brevity as a soft cap.

Backend consequence of (1): `POST /kids-yoga/freeze` now accepts either a `session_id` (the normal case - attach to an already-scheduled, content-less session) or falls back to `scheduled_date`/`scheduled_time` find-or-create (kept for a one-off session outside the recurring slot, e.g. something an admin adds by hand). Only 409s when the target session *already has* a lesson, not merely because a session row exists for that date - the earlier version would have incorrectly blocked every freeze once the recurring template existed, since an empty session is always there waiting. `GET /sessions/my` now includes a lightweight `kids_yoga_lesson: { id }` so the frontend can tell which upcoming sessions still need content.

Verified on dev with **one real Anthropic API call** (Indian Mythology / Journey of Hanuman / Diwali): got back a well-written, appropriately concise, age-appropriate lesson plan with a genuinely useful anti-repetition summary; froze it and confirmed the `.txt` file landed inside the actual "Kids Yoga Sessions" Drive folder (checked via the file's `parents` field, not just a returned link); confirmed via `GET /sessions/:id`, `GET /sessions/my`, and `GET /kids-yoga/usage` (1/3 used). The Weekly Schedule slot (Mon-Fri 17:00, topic Kids Yoga) was added on the dev test trainer account via the existing admin API - no new code needed for that part. Test session/lesson intentionally left live on dev for the user to click through themselves.

Deployed to dev (`tdev.onenessyoga.in`) - migration applied, `GOOGLE_KIDS_YOGA_FOLDER_ID` set, backend restarted.

### Follow-up fixes after further dev testing (2026-09-06)

- **Cultural Context made optional** (frontend, backend validation, and the DB column - migration `20260906190000_kids_yoga_cultural_context_optional`) - most days aren't tied to a festival, so requiring a choice every time was wrong. The prompt substitutes "no specific festival - keep it general and fun" when omitted. Also added a free-text "Other (specify)..." option so a trainer can name a specific festival or auspicious day not in the curated list.
- **Narrative Background expanded from 6 to 15 options**, each with 6 curated Primary Themes (was 3-5) - added Ocean & Underwater, Space & Galaxy, Village Life, Superheroes, Fairy Tales, Farm Life, Circus & Carnival, Pirates & Treasure Hunt, Music & Dance.
- **Target Age Range simplified** to three bands: 3-5, 6-10, 11-13 years.
- **Real generation failures found and fixed**: (1) a live "AI response was not valid JSON" turned out to be `max_tokens` (2200, lowered in the earlier brevity change) being too tight once the response also had to fit valid JSON escaping - raised back to 3500 as a safety ceiling, since the prompt's own word targets (not this cap) are what actually control length; (2) that still weren't enough because **Claude Sonnet 5 runs adaptive extended thinking by default even without requesting it**, and thinking tokens count against `max_tokens` - on this plain formulaic writing task (no real reasoning needed), that was silently eating the budget meant for the answer. Fixed by passing `thinking: { type: 'disabled' }` on this call. Also hardened `parseLessonResponse` to extract just the `{...}` span instead of requiring the whole trimmed string to be valid JSON, as a safety net against any stray text the model might add around it. Verified the extraction logic against 5 cases (clean JSON, leading/trailing prose, code fences) locally.
